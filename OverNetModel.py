...
class OverNet(nn.Module):
    ...
    def forward(self, img):
        ...
        h=self.OSMBlock(h)
        f_x4_1=F.interpolate(h, size=[img.shape[2]*4, img.shape[3]*4], mode="bicubic",align_corners=True)
        f_x4_2=F.interpolate(img, size=[img.shape[2]*4, img.shape[3]*4], mode="bicubic",align_corners=True)
        out=f_x4_1+f_x4_2
        return out
